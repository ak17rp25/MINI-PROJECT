...
def update_graph_scatter():
    ...
    if(compare==1):
    	...
    	if current=='infosys':
    		return {'data': [data1],'layout' : go.Layout(xaxis=dict(range=[min(X_info),max(X_info)],title="Interval(5-Second)"),
	                                                yaxis=dict(range=[min(Y_info)-0.8,max(Y_info)+0.8],title="Price"),
	                                                )}
    	elif current=='reliance':
    		return {'data': [data2],'layout' : go.Layout(xaxis=dict(range=[min(X_reli),max(X_reli)],title="Interval(5-Second)"),
	                                                yaxis=dict(range=[min(Y_reli)-0.8,max(Y_reli)+0.8],title="Price"),
	                                                )}
    	elif current=='wipro':
    		return {'data': [data3],'layout' : go.Layout(xaxis=dict(range=[min(X_wipro),max(X_wipro)],title="Interval(5-Second)"),
	                                            yaxis=dict(range=[min(Y_wipro)-0.8,max(Y_wipro)+0.8],title="Price"),
	                                            )}
    	elif current=='itc':
    		return {'data': [data4],'layout' : go.Layout(xaxis=dict(range=[min(X_itc),max(X_itc)],title="Interval(5-Second)"),
